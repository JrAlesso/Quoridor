    ...
    function usernameToAuthEmail(username) {
      ...
      return 'u' + hex + '@quoridor-online-2823.firebaseapp.com';
    }
    ...
    async function createUser(username, password) {
      ...
      try {
        // O Firebase Authentication impede que o mesmo
        // nome seja cadastrado novamente em outro navegador.
        var credential = await firebase.auth().createUserWithEmailAndPassword(
          email,
          password
        );
        ...
        // Mantém os dados locais para que o restante do jogo,
        // que ainda usa getUserStats(), continue funcionando.
    ...
    async function loginUser(username, password) {
      ...
      try {
        var credential = await firebase.auth().signInWithEmailAndPassword(
          email,
          password
        );
        ...
        // Primeiro tenta buscar os dados globais da conta.
        ...
        if (doc.exists) {
          ...
          accounts[username] = {
            ...
            normalizedUsername: data.normalizedUsername || username.toLowerCase(),
            passwordHash: '',
            stats: stats
          };
          ...
          // Caso a autenticação exista mas o documento Firestore
          // ainda não exista, cria o perfil.
          var oldAccount = accounts[username];
          var stats = oldAccount && oldAccount.stats
            ? oldAccount.stats
            : defaultUserStats();
        ...
        // Migração de uma conta antiga que ainda estava
        // somente no localStorage deste navegador.
        if (
          error &&
          (
          ...
          if (
            ...
            try {
              ...
              var newUser = newCredential.user;
              var stats = oldAccount.stats || defaultUserStats();
              ...
              console.error(
                'Erro ao migrar conta antiga:',
                migrationError
              );

              if (
                migrationError.code === 'auth/email-already-in-use'
              ) {
                // A conta já existe no Firebase.
