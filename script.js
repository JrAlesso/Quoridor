// Autenticação anônima ao carregar
...
// ============================================================
// SKINS PRÉ-DEFINIDAS (com categorias)
// ============================================================
var SKINS = [
  ...
  { id: 'espectral', nome: 'Espectral', forma: 'circle', cor1: '#ffffff', cor2: '#d3d3d3', preco: 9000, raridade: 'rara', efeito: 'ghost', categoria: 'raras' },
  { id: 'rei', nome: 'Rei', forma: 'diamond', cor1: '#ffd700', cor2: '#b8860b', preco: 10000, raridade: 'rara', efeito: 'crown', categoria: 'raras' }
];

// ============================================================
// SISTEMA DE PATENTES COM SUBDIVISÕES
// ============================================================
var PATENTES = [
  { nome: 'Bronze', subdivisoes: 5, pontosPorSub: 50 },
  { nome: 'Prata', subdivisoes: 5, pontosPorSub: 60 },
  { nome: 'Ouro', subdivisoes: 5, pontosPorSub: 70 },
  { nome: 'Platina', subdivisoes: 5, pontosPorSub: 80 },
  { nome: 'Diamante', subdivisoes: 5, pontosPorSub: 90 },
  { nome: 'Mestre', subdivisoes: 5, pontosPorSub: 100 },
  { nome: 'Cristal', subdivisoes: 5, pontosPorSub: 110 },
  { nome: 'Obsidiana', subdivisoes: 5, pontosPorSub: 120 },
  { nome: 'Eclipse', subdivisoes: 5, pontosPorSub: 130 },
  { nome: 'Lendário', subdivisoes: 1, pontosPorSub: Infinity }
];

// ============================================================
// FUNÇÕES DE AUTENTICAÇÃO
// ============================================================
function hashPassword(pass){ return btoa(unescape(encodeURIComponent(pass))); }
...
function createUser(username, password){
  ...
  accounts[username] = { passwordHash: hashPassword(password), stats: { games:0, wins:0, losses:0, streak:0, maxStreak:0, level:1, points:200, rankPoints:0, maxRankPoints:0, rank:'Bronze V', history:[], totalWalls:0, totalTurns:0, sumPointsVictories:0, opponentEloSum:0, opponentCount:0, xp:0, equippedTitle:'Recruta', ownedSkins:['classic'], equippedSkin:'classic', localGames:0, expertWins:0, medals:[] } };
  saveAccounts(accounts); return true;
}
function loginUser(username, password){
  var accounts = getAccounts();
  if(!accounts[username]) return false;
  if(accounts[username].passwordHash){
    if(accounts[username].passwordHash === hashPassword(password)){ currentUser = username; return true; }
  } else if(accounts[username].password === password){
    accounts[username].passwordHash = hashPassword(password);
    delete accounts[username].password;
    saveAccounts(accounts);
    currentUser = username; return true;
  }
  return false;
}
function getUserStats(username){
  var accounts = getAccounts();
  return accounts[username] && accounts[username].stats ? accounts[username].stats : { games:0, wins:0, losses:0, streak:0, maxStreak:0, level:1, points:0, rankPoints:0, maxRankPoints:0, rank:'Bronze V', history:[], totalWalls:0, totalTurns:0, sumPointsVictories:0, opponentEloSum:0, opponentCount:0, xp:0, equippedTitle:'Recruta', ownedSkins:['classic'], equippedSkin:'classic', localGames:0, expertWins:0, medals:[] };
}
function updateUserStats(username, newStats){
  var accounts = getAccounts();
  if(accounts[username]){ accounts[username].stats = newStats; saveAccounts(accounts); }
}
...
function obterPatente(points){
  ...
  for(var i=0;i<PATENTES.length;i++){
    ...
    if(points < acumulado + total || p.nome === 'Lendário'){
  ...
  return { nome: 'Lendário', sub: '', nomeCompleto: 'Lendário' };
}
function obterProximaPatente(points){
  var atual = obterPatente(points);
  if(atual.nome === 'Lendário') return null;
  var acumulado = 0;
  for(var i=0;i<PATENTES.length;i++){
    var p = PATENTES[i];
    var total = p.subdivisoes * p.pontosPorSub;
    if(points < acumulado + total){
      ...
      return next.nome === 'Lendário' ? 'Lendário' : next.nome + ' V';
    }
    acumulado += total;
  }
  return null;
...
// ============================================================
// FUNÇÕES DE LOGIN GLOBAIS (para onsubmit e onclick)
// ============================================================
window.handleLogin = function(e) {
  ...
  var isLoginMode = btn && btn.textContent === 'ENTRAR';
  ...
  if (isLoginMode) {
    ...
    if (createUser(username, password)) {
      ...
      if (errorDiv) errorDiv.textContent = 'Usuário já existe';
      return false;
    }
  }
};
...
window.toggleMode = function() {
  var btn = document.getElementById('login-btn');
  var toggle = document.getElementById('toggle-login');
  var confirmField = document.getElementById('login-confirm');
  if (btn) {
    if (btn.textContent === 'CRIAR CONTA') {
      btn.textContent = 'ENTRAR';
      if (toggle) toggle.textContent = 'Não tem conta? Cadastre-se';
      if (confirmField) confirmField.style.display = 'none';
    } else {
      btn.textContent = 'CRIAR CONTA';
      if (toggle) toggle.textContent = 'Já tem conta? Faça login';
...
// ============================================================
// SKINS ANIMADAS (DRAWSKINPREVIEW)
// ============================================================
function drawSkinPreview(canvas, skin) {
  ...
  function animate(time) {
    ...
    var t = time / 1000;
    var grad = ctx.createRadialGradient(cx - rad*0.3, cy - rad*0.35, rad*0.05, cx, cy, rad);
    ...
    ctx.shadowBlur = 8;
    ctx.beginPath();
    var forma = skin.forma || 'circle';
    if (forma === 'dragon') {
      ctx.ellipse(cx, cy + rad*0.1, rad*0.7, rad*0.85, 0, 0, Math.PI*2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(cx - rad*0.3, cy);
      ctx.quadraticCurveTo(cx - rad*1.2, cy - rad*0.7, cx - rad*0.5, cy - rad*1.0);
      ctx.quadraticCurveTo(cx - rad*0.8, cy - rad*0.3, cx - rad*0.3, cy);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(cx + rad*0.3, cy);
      ctx.quadraticCurveTo(cx + rad*1.2, cy - rad*0.7, cx + rad*0.5, cy - rad*1.0);
      ...
      switch(forma) {
        ...
        case 'square': ctx.rect(cx-rad, cy-rad, rad*2, rad*2); break;
        case 'diamond': ctx.moveTo(cx, cy-rad); ctx.lineTo(cx+rad, cy); ctx.lineTo(cx, cy+rad); ctx.lineTo(cx-rad, cy); ctx.closePath(); break;
        case 'hexagon': for(var k=0;k<6;k++){ var a=Math.PI/6+k*Math.PI/3; var hx=cx+rad*Math.cos(a), hy=cy+rad*Math.sin(a); if(k===0) ctx.moveTo(hx,hy); else ctx.lineTo(hx,hy); } ctx.closePath(); break;
        case 'star': for(var k=0;k<10;k++){ var a=-Math.PI/2+k*Math.PI/5; var r=k%2===0?rad:rad*0.5; var sx=cx+r*Math.cos(a), sy=cy+r*Math.sin(a); if(k===0) ctx.moveTo(sx,sy); else ctx.lineTo(sx,sy); } ctx.closePath(); break;
        default: ctx.arc(cx, cy, rad, 0, Math.PI*2);
      }
      ctx.fill();
      ctx.shadowBlur = 0;
    ...
    var efeito = skin.efeito || '';
    if (efeito === 'fire' || efeito === 'volcano' || efeito === 'inferno' || efeito === 'lava' || efeito === 'meteor') {
      var cor = efeito==='volcano'?'#ff4500':efeito==='inferno'?'#ff0000':efeito==='lava'?'#ff4500':efeito==='meteor'?'#ff8c00':'#ff4500';
      for(var i=0;i<8;i++){ var a=t*3+i*0.5; var d=Math.sin(t*5+i)*rad*0.5; var fx=cx+Math.cos(a)*d; var fy=cy-rad-i*1.8+Math.sin(t*8+i)*2.5; ctx.fillStyle=i%2===0?cor:'#ffd700'; ctx.globalAlpha=0.7-i*0.05; ctx.beginPath(); ctx.arc(fx,fy,1.5+Math.sin(t+i)*0.5,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
    } else if (efeito === 'dragon') {
      for(var i=0;i<10;i++){ var a=t*2.5+i*0.35; var d=Math.sin(t*4+i)*rad*0.6+rad*0.3; var fx=cx+Math.cos(a)*d*0.6; var fy=cy-rad*0.7-i*1.2+Math.sin(t*6+i)*3; ctx.fillStyle=i%3===0?'#ffd700':'#ff4500'; ctx.globalAlpha=0.8-i*0.04; ctx.beginPath(); ctx.arc(fx,fy,1.5+Math.sin(t*5+i)*0.5,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
    } else if (efeito === 'blackhole') {
      for(var i=0;i<20;i++){ var a=t*1.8+i*(Math.PI*2/20); var r1=rad*0.9+Math.sin(t*3+i)*rad*0.08; var r2=rad*1.2+Math.cos(t*2.5+i)*rad*0.1; var c1=cx+Math.cos(a)*r1; var c2=cy+Math.sin(a)*r1*0.55; var c3=cx+Math.cos(a+0.15)*r2; var c4=cy+Math.sin(a+0.15)*r2*0.55; ctx.strokeStyle=i%2===0?'rgba(255,140,0,0.5)':'rgba(180,40,255,0.4)'; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(c1,c2); ctx.lineTo(c3,c4); ctx.stroke(); } ctx.strokeStyle='rgba(255,200,80,0.5)'; ctx.lineWidth=1.5; ctx.beginPath(); ctx.ellipse(cx,cy,rad*0.8,rad*0.4,0,0,Math.PI*2); ctx.stroke();
    } else if (efeito === 'ice' || efeito === 'ice_dragon') {
      for(var i=0;i<6;i++){ var a=t*1.5+i*0.63; var d=rad*0.9+Math.sin(t*2+i)*rad*0.3; var ix=cx+Math.cos(a)*d; var iy=cy+Math.sin(a)*d; ctx.fillStyle='#e0ffff'; ctx.globalAlpha=0.6; ctx.beginPath(); ctx.arc(ix,iy,1.5+Math.sin(t+i)*0.5,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
    } else if (efeito === 'shadow') {
      for(var i=0;i<6;i++){ var a=t*1.2+i*0.8; var d=rad*0.9+Math.sin(t*3+i)*rad*0.4; var sx=cx+Math.cos(a)*d; var sy=cy+Math.sin(a)*d*0.6; ctx.strokeStyle='rgba(0,0,0,0.6)'; ctx.lineWidth=1.5; ctx.beginPath(); ctx.moveTo(cx,cy); ctx.quadraticCurveTo(cx+Math.cos(a)*d*0.5,cy+Math.sin(a)*d*0.3,sx,sy); ctx.stroke(); }
    } else if (efeito === 'nature' || efeito === 'floresta') {
      for(var i=0;i<8;i++){ var a=t*0.8+i*0.52; var d=rad*0.9+Math.sin(t*2+i)*rad*0.3; var nx=cx+Math.cos(a)*d; var ny=cy+Math.sin(a)*d; ctx.fillStyle=i%2===0?'#32cd32':'#228b22'; ctx.globalAlpha=0.6; ctx.beginPath(); ctx.arc(nx,ny,1.8,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
    } else if (efeito === 'cosmic_eye') {
      ctx.strokeStyle='#ff00ff'; ctx.lineWidth=1.5; ctx.beginPath(); ctx.arc(cx,cy,rad*0.9,0,Math.PI*2); ctx.stroke(); ctx.fillStyle='#ff00ff'; ctx.beginPath(); ctx.arc(cx,cy,rad*0.15,0,Math.PI*2); ctx.fill(); var ea=t*2; ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(cx+Math.cos(ea)*rad*0.3,cy+Math.sin(ea)*rad*0.3,rad*0.1,0,Math.PI*2); ctx.fill();
    } else if (efeito === 'wind') {
      for(var i=0;i<8;i++){ var a=t*4+i*(Math.PI*2/8); var d=rad*0.9+Math.sin(t*8+i)*rad*0.3; var x1=cx+Math.cos(a)*d; var y1=cy+Math.sin(a)*d*0.5; ctx.strokeStyle='rgba(255,255,255,0.6)'; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x1+Math.cos(a)*6,y1+Math.sin(a)*3); ctx.stroke(); }
    } else if (efeito === 'arcane_mage') {
      for(var i=0;i<6;i++){ var a=t*1.2+i*(Math.PI*2/6); var d=rad; var mx=cx+Math.cos(a)*d; var my=cy+Math.sin(a)*d; ctx.fillStyle='#8a2be2'; ctx.globalAlpha=0.5+Math.sin(t*3+i)*0.3; ctx.font='10px Arial'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('✦',mx,my); } ctx.globalAlpha=1;
    } else if (efeito === 'pandora') {
      for(var i=0;i<6;i++){ var a=t*2+i*0.63; var d=rad*0.9+Math.sin(t*5+i)*rad*0.4; var px=cx+Math.cos(a)*d; var py=cy+Math.sin(a)*d; ctx.fillStyle=['#ffd700','#ff4500','#00ffff','#ff00ff'][i%4]; ctx.globalAlpha=0.7; ctx.beginPath(); ctx.arc(px,py,1.5,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
    } else if (efeito === 'serpent') {
      for(var i=0;i<6;i++){ var a=t*1.5+i*0.8; var d=rad*0.9; var sx=cx+Math.cos(a)*d; var sy=cy+Math.sin(a)*d; ctx.strokeStyle='#32cd32'; ctx.lineWidth=1.2; ctx.beginPath(); ctx.moveTo(cx,cy); ctx.quadraticCurveTo(cx+Math.cos(a)*d*0.5,cy+Math.sin(a)*d*0.3,sx,sy); ctx.stroke(); }
    } else if (efeito === 'water' || efeito === 'ocean') {
      for(var i=0;i<6;i++){ var a=t*1.5+i*0.63; var d=rad*0.9+Math.sin(t*2+i)*rad*0.3; var wx=cx+Math.cos(a)*d; var wy=cy+Math.sin(a)*d; ctx.fillStyle='#00bfff'; ctx.globalAlpha=0.5; ctx.beginPath(); ctx.arc(wx,wy,1.8,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
    } else if (efeito === 'sand' || efeito === 'deserto') {
      for(var i=0;i<6;i++){ var a=t*2+i*0.63; var d=rad*0.9+Math.sin(t*3+i)*rad*0.3; var sx=cx+Math.cos(a)*d; var sy=cy+Math.sin(a)*d; ctx.fillStyle='#edc9af'; ctx.globalAlpha=0.5; ctx.beginPath(); ctx.arc(sx,sy,1.2,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
    } else if (efeito === 'sky' || efeito === 'ceu') {
      for(var i=0;i<6;i++){ var a=t*1.5+i*0.8; var d=rad*0.9; var sx=cx+Math.cos(a)*d; var sy=cy+Math.sin(a)*d*0.5; ctx.fillStyle='#87ceeb'; ctx.globalAlpha=0.4; ctx.beginPath(); ctx.arc(sx,sy,1.5,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
    } else if (efeito === 'ghost' || efeito === 'espectral') {
      for(var i=0;i<6;i++){ var a=t*1.5+i*1.0; var d=rad*0.9; var gx=cx+Math.cos(a)*d; var gy=cy+Math.sin(a)*d*0.6; ctx.strokeStyle='rgba(255,255,255,0.5)'; ctx.lineWidth=1; ctx.beginPath(); ctx.arc(gx,gy,2,0,Math.PI*2); ctx.stroke(); }
    } else if (efeito === 'crown' || efeito === 'rei') {
      ctx.fillStyle='#ffd700'; ctx.beginPath(); ctx.moveTo(cx-rad*0.4,cy-rad*0.5); ctx.lineTo(cx-rad*0.4,cy-rad*0.8); ctx.lineTo(cx-rad*0.15,cy-rad*0.65); ctx.lineTo(cx,cy-rad*0.9); ctx.lineTo(cx+rad*0.15,cy-rad*0.65); ctx.lineTo(cx+rad*0.4,cy-rad*0.8); ctx.lineTo(cx+rad*0.4,cy-rad*0.5); ctx.closePath(); ctx.fill();
    }

    ctx.shadowBlur = 0;
    ctx.fillStyle = '#fff';
    ctx.font = 'bold ' + Math.round(rad*0.65) + 'px Arial';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('1', cx, cy + (forma==='dragon'?rad*0.25:0));
...
function openSkins(){ renderSkinsList(); document.getElementById('skins-overlay').classList.add('show'); }
function closeSkins(){ document.getElementById('skins-overlay').classList.remove('show'); }
...
function renderSkinsList() {
  ...
  for (var i=0; i<filteredSkins.length; i++) {
    ...
    if (!owned) { btn.className = 'skin-btn buy'; btn.textContent = 'Comprar'; btn.setAttribute('data-skin-id', skin.id); }
    else if (equipped) { btn.className = 'skin-btn equipped'; btn.disabled = true; btn.textContent = '✓ Em uso'; }
    else { btn.className = 'skin-btn equip'; btn.textContent = 'Equipar'; btn.setAttribute('data-skin-id', skin.id); }
...
function renderMedalsInProfile(listElement, stats) {
  ...
  var medalsContainer = document.createElement('div');
  medalsContainer.style.cssText = 'padding:12px;border-top:1px solid rgba(255,215,140,0.06);border-bottom:1px solid rgba(255,215,140,0.06);margin:10px 0;';
  medalsContainer.innerHTML = '<div style="font-weight:700;color:#d4a373;margin-bottom:8px;font-size:11px;">MEDALHAS</div>';
  var medalRow = document.createElement('div');
  medalRow.style.cssText = 'display:flex;gap:10px;flex-wrap:wrap;';
  for (var i = 0; i < MEDALS.length; i++) {
    ...
    (function(m){ medalDiv.addEventListener('click', function(){ alert(m.nome + ': ' + m.desc); }); })(medal);
    medalRow.appendChild(medalDiv);
  }
  medalsContainer.appendChild(medalRow);
  listElement.appendChild(medalsContainer);
}
function openMedals() {
  var stats = getUserStats(currentUser);
  ...
  for (var i = 0; i < MEDALS.length; i++) {
    ...
    item.innerHTML = '<span class="medal-svg">' + medal.svg(unlocked) + '</span>' +
      '<div class="medal-info"><div class="medal-name">' + medal.nome + '</div>' +
      '<div class="medal-req">' + medal.desc + '</div></div>' +
      '<span class="title-status">' + (unlocked ? '✔' : '🔒') + '</span>';
    list.appendChild(item);
  }
  document.getElementById('medals-overlay').classList.add('show');
}
function closeMedals() { document.getElementById('medals-overlay').classList.remove('show'); }
...
function closeRanks(){ document.getElementById('ranks-overlay').classList.remove('show'); }
...
var IA_WINS_TITLES = [
  ...
  { minVsIAWins: 100, title: 'Preditor' },
...
function renderTitleList(listElement, arr, key, iconUnlocked, iconLocked, reqPrefix){
  ...
  for(var i=0;i<arr.length;i++){
    ...
    item.innerHTML = '<span class="title-icon">' + (unlocked ? iconUnlocked : '🔒') + '</span>' +
      '<div class="title-info"><div class="title-name">' + t.title + '</div>' +
      '<div class="title-req">' + reqPrefix + ' ' + threshold + '</div></div>' +
...
function renderCurrentTitlesTab(){
  ...
  else if(currentTitlesTab === '2p') renderTitleList(list, LOCAL_GAMES_TITLES, 'localGames', '⚔️', '🔒', 'Requer');
  else if(currentTitlesTab === 'vsia') renderTitleList(list, IA_WINS_TITLES, 'expertWins', '🤖', '🔒', 'Requer');
  else list.innerHTML = '<div class="history-empty" style="color:#b8a99a;padding:30px 0;text-align:center;">🏟️ Em breve</div>';
}
function openTitles(){
...
function closeTitles(){ document.getElementById('titles-overlay').classList.remove('show'); document.getElementById('title-search').value=''; titleSearchQuery=''; }
...
// ============================================================
// RENDERIZAÇÃO DE HISTÓRICO E PERFIL
// ============================================================
function renderHistory(){
  var stats = getUserStats(currentUser);
  var list = document.getElementById('history-list');
  list.innerHTML = '';
  if(!stats.history || stats.history.length === 0){ list.innerHTML = '<div class="history-empty">Nenhuma partida ainda.</div>'; return; }
...
function showHistory(){ document.getElementById('history-overlay').classList.add('show'); renderHistory(); }
...
function closeProfile(){ document.getElementById('profile-overlay').classList.remove('show'); document.querySelectorAll('.profile-item').forEach(function(el){ el.classList.remove('active'); }); }

// ============================================================
// SISTEMA DE XP E PONTOS
// ============================================================
function xpParaProximoNivel(nivel){ return Math.floor(100 * Math.pow(nivel, 1.5)); }
function calcularXP(winner, playerIndex, wallsPlaced, moveCount, timeLeft, timeTotal, streak){
  ...
  if(winner !== playerIndex){ bonusWall=Math.min(bonusWall,5); bonusMove=Math.min(bonusMove,5); bonusTime=0; bonusStreak=0; }
...
function goToModeScreen(username){
  currentUser = username;
  var stats = getUserStats(username);
  if(username === 'Akl'){ stats.points = 100000; updateUserStats(username, stats); }
  document.getElementById('user-name').textContent = username;
  document.getElementById('user-avatar').textContent = username[0].toUpperCase();
  document.getElementById('stat-games-mode').textContent = stats.games;
  document.getElementById('stat-wins-mode').textContent = stats.wins;
...
function hideAllOverlays(){
  ['win-overlay','config-overlay','stats-overlay','profile-overlay','history-overlay','titles-overlay','inspect-overlay','skins-overlay','ranks-overlay','medals-overlay'].forEach(function(id){ document.getElementById(id).classList.remove('show'); });
...
function resetAllGameState(){
  ...
  hideAllOverlays();
  G = {pos:[[8,4],[0,4]],walls:[10,10],turn:0,pH:[],pV:[],wallOwnerH:[],wallOwnerV:[],hist:[],over:false,sel:null,moves:[],mode:'move',validH:[],validV:[],hoverNode:null,nivelIA:G.nivelIA||'medio',iaThinking:false,p1Name:currentUser||'Player 1',p2Name:'Player 2',vsIA:G.vsIA};
...
// ============================================================
// LÓGICA DO JOGO (TABULEIRO, IA, TIMER, ETC.)
// ============================================================
...
var G = {pos:[[8,4],[0,4]],walls:[WALLS,WALLS],turn:0,pH:[],pV:[],wallOwnerH:[],wallOwnerV:[],hist:[],over:false,sel:null,moves:[],mode:'move',validH:[],validV:[],hoverNode:null,nivelIA:'medio',iaThinking:false,p1Name:'Player 1',p2Name:'Player 2',vsIA:false};
var config = {time:30, rounds:1};
var scores=[0,0], currentRound=1, timerInterval=null, currentTime=30, timerRunning=false, gameActive=false, resetPending=false, matchFinished=false;
...
function getStats(){
  if(currentUser){
    ...
    return {games:s.games, winsP1:s.wins, losses:s.losses||0, streak:s.streak||0, maxStreak:s.maxStreak||0, rankPoints:s.rankPoints||0, maxRankPoints:s.maxRankPoints||0, history:s.history||[], totalWalls:s.totalWalls||0, totalTurns:s.totalTurns||0, sumPointsVictories:s.sumPointsVictories||0, opponentEloSum:s.opponentEloSum||0, opponentCount:s.opponentCount||0, level:s.level||1, xp:s.xp||0, equippedTitle:s.equippedTitle||'Recruta', ownedSkins:s.ownedSkins||['classic'], equippedSkin:s.equippedSkin||'classic', localGames:s.localGames||0, expertWins:s.expertWins||0, medals:s.medals||[]};
  }
  return {games:0,winsP1:0,losses:0,streak:0,maxStreak:0,rankPoints:0,maxRankPoints:0,history:[],totalWalls:0,totalTurns:0,sumPointsVictories:0,opponentEloSum:0,opponentCount:0,level:1,xp:0,equippedTitle:'Recruta', ownedSkins:['classic'], equippedSkin:'classic', localGames:0, expertWins:0, medals:[]};
}
function saveStats(stats){
  if(!currentUser) return;
  var userStats = getUserStats(currentUser);
  userStats.games = stats.games; userStats.wins = stats.winsP1; userStats.losses = stats.losses;
  userStats.streak = stats.streak; userStats.maxStreak = stats.maxStreak;
  userStats.rankPoints = stats.rankPoints; userStats.maxRankPoints = stats.maxRankPoints;
  userStats.history = stats.history; userStats.totalWalls = stats.totalWalls; userStats.totalTurns = stats.totalTurns;
  userStats.sumPointsVictories = stats.sumPointsVictories; userStats.opponentEloSum = stats.opponentEloSum;
  userStats.opponentCount = stats.opponentCount; userStats.level = stats.level; userStats.xp = stats.xp;
  ...
  userStats.rank = getRank(stats.rankPoints);
  updateUserStats(currentUser, userStats);
}

// ============================================================
// DRAW PAWN (COMPLETO - COM EFEITOS DE SKINS)
// ============================================================
...
// ============================================================
// FUNÇÕES DO TABULEIRO (COMPLETAS)
// ============================================================
function resize(){
...
function iaJogarExpert() {
  ...
  for (var depth = 2; depth <= cfg.depth; depth += 2) {
    ...
    if (!playerCenter && wallsUsed >= maxWallsToUse) {
      wCands = [];
    }
    var actions = [];
...
function wallCandidates(pH, pV, walls, pos, iaIdx) {
  if (walls[iaIdx] <= 0) return [];
  var cands = [];
  var opp = 1 - iaIdx;
  for (var r = 0; r < N - 1; r++) {
    for (var c = 0; c < N - 1; c++) {
      if (canPlaceIA(r, c, 'H', pH, pV, walls[iaIdx], pos)) {
        ...
        var newDist = bfsDist(opp, tH, pV, pos);
        var gain = newDist - bfsDist(opp, pH, pV, pos);
        if (gain >= 1) cands.push({r: r, c: c, ori: 'H', gain: gain});
      }
      if (canPlaceIA(r, c, 'V', pH, pV, walls[iaIdx], pos)) {
        var tV = pV.concat([[r, c]]);
        var newDist = bfsDist(opp, pH, tV, pos);
        var gain = newDist - bfsDist(opp, pH, pV, pos);
        if (gain >= 1) cands.push({r: r, c: c, ori: 'V', gain: gain});
      }
    }
  }
  cands.sort(function(a, b) { return b.gain - a.gain; });
...
function minimax(pos, pH, pV, walls, depth, alpha, beta, maximizing, iaIdx, startTime, timeLimit) {
  var key = hashState(pos, pH, pV);
  if (transpositionTable.has(key)) {
    var stored = transpositionTable.get(key);
    if (stored.depth >= depth) return stored.value;
  ...
  if (d1 === 0) return 100000 + depth;
  if (d0 === 0) return -100000 - depth;
  if (depth === 0 || Date.now() - startTime > timeLimit) return (d0 - d1) * 100;
  var cur = maximizing ? iaIdx : 1 - iaIdx;
  var moves = legalMoves(cur, pH, pV, pos);
  var wCands = wallCandidates(pH, pV, walls, pos, cur).slice(0, maximizing ? 50 : 30);
  var actions = [];
  for (var i = 0; i < moves.length; i++) {
    actions.push({type:'move', r:moves[i][0], c:moves[i][1], dist: Math.abs(moves[i][0] - WIN[cur])});
  }
  ...
  if (!actions.length) return (d0 - d1) * 100;
  var bestVal = maximizing ? -Infinity : Infinity;
  for (var k = 0; k < actions.length; k++) {
    ...
    var newKey = hashState(npos, npH, npV);
    if (isRecentState(newKey)) continue;
    var val = minimax(npos, npH, npV, nw, depth - 1, alpha, beta, !maximizing, iaIdx, startTime, timeLimit);
    if (maximizing) { if (val > bestVal) bestVal = val; alpha = Math.max(alpha, bestVal); }
    else { if (val < bestVal) bestVal = val; beta = Math.min(beta, bestVal); }
    if (beta <= alpha) break;
  }
...
// ============================================================
// DRAW, RR E FUNÇÕES DE INTERFACE
// ============================================================
function draw(){
...
function showWinOverlay(msg, vencedor){
  ...
  for(var i=0;i<100;i++){
    ...
    p.style.width=(Math.random()*10+4)+'px'; p.style.height=(Math.random()*14+6)+'px';
...
// ============================================================
// FUNÇÕES DE JOGO (MOVIMENTOS, PAREDES, TURNOS)
// ============================================================
...
function resetGame(){
  ...
  document.getElementById('confetti-container').innerHTML='';
  document.getElementById('btn-close-win').style.display='none';
  G = {pos:[[8,4],[0,4]],walls:[WALLS,WALLS],turn:0,pH:[],pV:[],wallOwnerH:[],wallOwnerV:[],hist:[],over:false,sel:null,moves:[],mode:'move',validH:[],validV:[],hoverNode:null,vsIA:G.vsIA,nivelIA:G.nivelIA,iaThinking:false,p1Name:G.p1Name,p2Name:G.p2Name};
...
function openInspect(playerIndex, viewer){
  ...
  if(playerIndex === 0 && currentUser){
    data = [
      {label:'Nome',value:name},{label:'Título',value:stats.equippedTitle||'Recruta'},
      {label:'Patente',value:getRank(stats.rankPoints)},{label:'Nível',value:stats.level||1},
      {label:'Partidas',value:stats.games},{label:'Vitórias',value:stats.winsP1},
      {label:'Derrotas',value:stats.losses},
      {label:'Taxa de vitórias',value:(stats.games>0?((stats.winsP1/stats.games)*100).toFixed(1)+'%':'0%')},
    ...
    data = [{label:'Nome',value:name},{label:'Tipo',value:'Inteligência Artificial'},{label:'Dificuldade',value:G.nivelIA.toUpperCase()}];
  } else {
    data = [{label:'Nome',value:name},{label:'Tipo',value:'Jogador Local'},{label:'Título',value:'Nenhum'}];
  }
  for(var i=0;i<data.length;i++){
    var item = document.createElement('div');
...
function closeInspect(){ document.getElementById('inspect-overlay').classList.remove('show'); document.getElementById('inspect-card').classList.remove('rotated'); }

// ============================================================
// EVENTOS DE CONFIGURAÇÃO E INICIALIZAÇÃO
// ============================================================
...
document.getElementById('btnStats').addEventListener('click', function(){
  var s = getStats();
  document.getElementById('stats-content').innerHTML = '<div style="color:#b8a99a;font-size:13px;line-height:1.8">Partidas: <b style="color:#f0e6d3">' + s.games + '</b><br>Vitórias: <b style="color:#10b981">' + s.winsP1 + '</b><br>Derrotas: <b style="color:#e94560">' + s.losses + '</b><br>Sequência: <b style="color:#d4a373">' + s.streak + '</b></div>';
...
console.log('✅ Quoridor Online carregado!');
</script>
<script>
(function(){
  var firebaseConfig={
    ...
    storageBucket:"quoridor-online-2823d.appspot.com",
    messagingSenderId:"652289533045",
    appId:"1:652289533045:web:043558b291"
  };
  try{if(!firebase.apps.length)firebase.initializeApp(firebaseConfig);}catch(e){}
  var _chatMsgsHandler=null,_chatSalasHandler=null,_chatSalaAtual=null;
  function getRtdb(){try{return firebase.database();}catch(e){alert(e.message);return null;}}
  function escapeHTML(str){return String(str).replace(/[&<>"]/g,function(m){if(m==='&')return'&amp;';if(m==='<')return'&lt;';if(m==='>')return'&gt;';if(m==='"')return'&quot;';return m;});}
  function carregarMensagens(salaId){
    if(!salaId)salaId="geral";
    _chatSalaAtual=salaId;
    ...
    if(_chatMsgsHandler){try{firebase.database().ref("chat/"+salaId).off("value",_chatMsgsHandler);}catch(e){}}
    var rtdb=getRtdb();if(!rtdb)return;
    _chatMsgsHandler=function(snap){
      container.innerHTML='';
      var val=snap.val()||{};
      var keys=Object.keys(val).sort();
      if(!keys.length){container.innerHTML='<div style="color:#666;text-align:center;padding:20px;">Nenhuma mensagem.</div>';return;}
      keys.slice(-50).forEach(function(k){
        var m=val[k];
        var div=document.createElement('div');div.className='msg';
        var date=m.t?new Date(m.t).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):'';
        div.innerHTML='<span class="user">'+escapeHTML(m.u||'Anonimo')+'</span><span class="time">'+date+'</span><br>'+escapeHTML(m.tx||'');
        container.appendChild(div);
      });
      container.scrollTop=container.scrollHeight;
    };
    rtdb.ref("chat/"+salaId).orderByKey().on("value",_chatMsgsHandler);
  }
  function enviarMensagem(){
    var input=document.getElementById("chat-input");if(!input)return;
    var txt=input.value.trim();if(!txt)return;
    var user=currentUser||'Anonimo';
    var sala=_chatSalaAtual||"geral";
    var rtdb=getRtdb();if(!rtdb)return;
    rtdb.ref("chat/"+sala).push({u:user,tx:txt,t:Date.now()});
    input.value='';
  }
  function carregarSalas(){
    var container=document.getElementById("chat-salas");if(!container)return;
    container.innerHTML='<div style="color:#666;text-align:center;padding:10px;">Carregando salas...</div>';
    if(_chatSalasHandler){try{firebase.database().ref("chat_salas").off("value",_chatSalasHandler);}catch(e){}}
    var rtdb=getRtdb();if(!rtdb)return;
    _chatSalasHandler=function(snap){
      container.innerHTML='';
      var val=snap.val()||{};
      var items=[];
      Object.keys(val).forEach(function(id){var d=val[id];if(d)items.push({id:id,d:d});});
      if(!items.length){container.innerHTML='<div style="color:#666;text-align:center;padding:10px;">Nenhuma sala privada.</div>';return;}
      items.forEach(function(it){
        var d=it.d,row=document.createElement('div');row.className='sala-item';
        var lock=d.senha?'🔒 ':'';
        row.innerHTML='<span class="sala-nome">'+lock+(d.nome||it.id)+'</span><span class="sala-users">👤 '+(d.usuarios||0)+'</span>';
        var btn=document.createElement('button');btn.className='btn-entrar-chat';btn.textContent='Entrar';
        btn.onclick=function(){entrarSalaPrivada(it.id,d);};
        row.appendChild(btn);container.appendChild(row);
      });
    };
    rtdb.ref("chat_salas").on("value",_chatSalasHandler);
  }
  function criarSalaPrivada(){
    var nome=document.getElementById("chat-sala-nome");if(!nome)return;
    var senha=document.getElementById("chat-sala-senha");if(!senha)return;
    var n=nome.value.trim()||"Sala";var s=senha.value.trim();
    var rtdb=getRtdb();if(!rtdb)return;
    var ref=rtdb.ref("chat_salas").push();
    ref.set({nome:n,senha:s||'',criador:currentUser||'Anonimo',usuarios:1,criado:Date.now()});
    alert("Sala criada! Entre nela.");
    nome.value='';senha.value='';
    carregarSalas();
  }
  function entrarSalaPrivada(salaId,data){
    if(data.senha){var p=prompt("Senha da sala:");if(p===null)return;if(p!==data.senha){alert("Senha incorreta");return;}}
    var rtdb=getRtdb();if(!rtdb)return;
    rtdb.ref("chat_salas/"+salaId+"/usuarios").transaction(function(cur){return (cur||0)+1;});
    document.querySelector('[data-chat-tab="geral"]').click();
    carregarMensagens(salaId);
    // Atualiza contador ao sair
    document.getElementById("chat-close").onclick=function(){
      rtdb.ref("chat_salas/"+salaId+"/usuarios").transaction(function(cur){return Math.max(0,(cur||1)-1);});
  ...
  function fecharChat(){
    document.getElementById("chat-overlay").classList.remove("show");
    if(_chatMsgsHandler){try{firebase.database().ref("chat/"+(_chatSalaAtual||"geral")).off("value",_chatMsgsHandler);}catch(e){}}
    if(_chatSalasHandler){try{firebase.database().ref("chat_salas").off("value",_chatSalasHandler);}catch(e){}}
  }
  // Tabs
  document.querySelectorAll('.chat-tab').forEach(function(tab){
    tab.addEventListener('click',function(){
      ...
      this.classList.add('active');
      var target=this.dataset.chatTab;
      document.querySelectorAll('.chat-panel').forEach(function(p){p.classList.remove('active');});
      document.getElementById('panel-'+target).classList.add('active');
      if(target==='geral')carregarMensagens('geral');
      if(target==='salas')carregarSalas();
    });
  });
  // Eventos
  document.getElementById('btn-chat').addEventListener('click',abrirChat);
  document.getElementById('chat-close').addEventListener('click',fecharChat);
  document.getElementById('btn-close-chat').addEventListener('click',fecharChat);
  document.getElementById('chat-send').addEventListener('click',enviarMensagem);
  document.getElementById('chat-input').addEventListener('keydown',function(e){if(e.key==='Enter')enviarMensagem();});
  document.getElementById('chat-criar-sala-btn').addEventListener('click',criarSalaPrivada);
  document.getElementById('chat-overlay').addEventListener('click',function(e){if(e.target===this)fecharChat();});
  console.log('✅ Chat Online ativado (público/privado)!');
})();
</script>
<script>
(function(){
  ...
  var isOnlineMode=false, salaAtual=null, _listaHandler=null, _jogoHandler=null;
  var meuJogadorIndex = 0; // 0 = jogador1, 1 = jogador2

  function getRtdb(){try{return firebase.database();}catch(e){alert(e.message);return null;}}
  function setStatus(m){var el=document.getElementById("sala-status");if(el)el.innerHTML=m;}
  ...
  function ehMinhaVez(){
    ...
    var dono = G.p1Name || '';
    var oponente = G.p2Name || '';
    if(currentUser === dono && G.turn === 0) return true;
    if(currentUser === oponente && G.turn === 1) return true;
    return false;
  }

  // ============================================================
  // AUTO SELEÇÃO DO PEÃO
  // ============================================================
  function autoPeao(){
    if(!isOnlineMode || !G || G.over || !gameActive) return;
    if(!ehMinhaVez()){
      G.sel = null;
      G.moves = [];
    ...
    var p = G.turn;
    G.sel = [G.pos[p][0], G.pos[p][1]];
    if(typeof legalMoves === "function") G.moves = legalMoves(p);
    if(typeof st === "function") st("Sua vez");
    if(typeof draw === "function") draw();
  }
  ...
  function escutarSalas(){
    var rtdb=getRtdb(); if(!rtdb) return;
    if(_listaHandler){ try{rtdb.ref("salas").off("value",_listaHandler);}catch(e){} }
    _listaHandler=function(snap){
      var lista=document.getElementById("sala-lista"); if(!lista) return;
      lista.innerHTML="";
      var val=snap.val()||{}, items=[];
      Object.keys(val).forEach(function(id){
        var d=val[id];
        if(d && d.status==="esperando" && !d.jogador2) items.push({id:id, d:d});
      });
      items.sort(function(a,b){ return (b.d.createdAt||0)-(a.d.createdAt||0); });
      if(!items.length){
        lista.innerHTML='<div class="history-empty">Nenhuma sala disponível. Crie uma!</div>';
        return;
      }
      items.forEach(function(it){
        var d=it.d, nome=d.nomePartida||d.jogador1||"Sala";
        var lock=d.senha?"🔒 ":"", souEu=d.jogador1===currentUser;
        var row=document.createElement("div");
        row.className="sala-item";
        row.innerHTML='<div class="sala-info"><span class="sala-nome">'+lock+nome+'</span><br><span class="sala-host">👤 '+(d.jogador1||"")+(souEu?" (sua)":"")+'</span></div>';
        if(!souEu){
          var btn=document.createElement("button");
    ...
    rtdb.ref("salas").orderByChild("createdAt").on("value",_listaHandler,function(err){ setStatus(err.message); });
  }
  ...
  function criarSala(){
    ...
    var ref=rtdb.ref("salas").push(), id=ref.key;
    setStatus("Criando...");
    ref.set({
      nomePartida: nome,
      ...
      ref.on("value", function(snap){
        var data=snap.val();
        if(data && data.status==="em_andamento" && data.jogador2){
          document.getElementById("sala-overlay").classList.remove("show");
          iniciarJogoOnline(id, data);
        }
      });
      escutarSalas();
  ...
  function entrarSala(salaId, data){
    ...
    if(data.senha){
      ...
      if(s===null) return;
      if(s!==data.senha){ alert("Senha incorreta"); return; }
    }
    var rtdb=getRtdb(), ref=rtdb.ref("salas/"+salaId);
    ref.transaction(function(cur){
      if(!cur) return cur;
      if(cur.status!=="esperando" || (cur.jogador2 && cur.jogador2!=="")) return;
      cur.jogador2 = currentUser;
      cur.status = "em_andamento";
      return cur;
    }).then(function(r){
      if(!r.committed){ alert("Sala já ocupada"); escutarSalas(); return; }
      document.getElementById("sala-overlay").classList.remove("show");
      iniciarJogoOnline(salaId, r.snapshot.val());
  ...
  function aplicarEstado(data){
    ...
    G.p2Name = data.jogador2 || G.p2Name;
    G.pos = data.posicoes || G.pos;
    G.walls = data.paredesRestantes || G.walls;
    G.turn = (typeof data.turno === "number") ? data.turno : G.turn;
    G.pH = [];
    G.pV = [];
    G.wallOwnerH = [];
    G.wallOwnerV = [];
    (data.paredesH||[]).forEach(function(w){
      if(Array.isArray(w)){ G.pH.push([w[0],w[1]]); G.wallOwnerH.push(0); }
      else if(w){ G.pH.push([w.r,w.c]); G.wallOwnerH.push(w.dono||0); }
    });
    (data.paredesV||[]).forEach(function(w){
      if(Array.isArray(w)){ G.pV.push([w[0],w[1]]); G.wallOwnerV.push(0); }
      else if(w){ G.pV.push([w.r,w.c]); G.wallOwnerV.push(w.dono||0); }
    });
    // Atualiza nomes na interface
    var a=document.getElementById("p1-nome"); if(a) a.textContent=G.p1Name;
    var b=document.getElementById("nomeJ2"); if(b) b.textContent=G.p2Name;
  ...
  // INICIAR JOGO ONLINE
  // ============================================================
  function iniciarJogoOnline(salaId, data){
    isOnlineMode = true;
    salaAtual = salaId;
    G.vsIA = false;
    G.over = false;
    ...
    matchFinished = false;
    aplicarEstado(data);
    // Define quem é o jogador local
    if(currentUser === G.p1Name) meuJogadorIndex = 0;
    else if(currentUser === G.p2Name) meuJogadorIndex = 1;
    ...
    // Escuta mudanças no jogo
    var rtdb=getRtdb(), ref=rtdb.ref("salas/"+salaId);
    if(_jogoHandler){ try{ ref.off("value", _jogoHandler); }catch(e){} }
    _jogoHandler = function(snap){
      var novo=snap.val();
      if(!novo) return;
      if(novo.status==="finalizada"){
        if(!G.over){
          G.over = true;
          var vencedor = novo.vencedor || "";
          var idx = (vencedor === G.p1Name) ? 0 : 1;
          if(typeof showWinOverlay === "function"){
            showWinOverlay("◈ " + vencedor + " venceu!", idx);
          }
        }
        return;
      }
  ...
  function enviarEstado(){
    ...
    // Só envia se for a vez do jogador local
  ...
  // ============================================================
  // SOBRESCREVER FUNÇÕES DO JOGO PARA ENVIAR ESTADO
  // ============================================================
  if(typeof doMove === "function"){
    var _dm = doMove;
    doMove = function(r,c){
      if(isOnlineMode && !ehMinhaVez()){
        if(typeof st === "function") st("Não é sua vez");
        return;
      }
      var o = _dm.apply(this, arguments);
      if(isOnlineMode) enviarEstado();
      setTimeout(autoPeao, 50);
      return o;
    };
  }
  ...
  if(typeof placeWall === "function"){
    ...
    placeWall = function(ni,nj,ori){
      ...
      var o = _pw.apply(this, arguments);
      if(isOnlineMode) enviarEstado();
      setTimeout(autoPeao, 50);
      return o;
    };
  ...
  function abrir(){
    ...
    setStatus("Crie ou entre numa sala");
    escutarSalas();
  }
  ...
  bind("btn-close-sala", function(){
    document.getElementById("sala-overlay").classList.remove("show");
  });
  bind("sala-close", function(){
    document.getElementById("sala-overlay").classList.remove("show");
  });

  console.log("✅ ONLINE CORRIGIDO ativo (turnos sincronizados)!");
})();
</script>
<script>
(function(){
  ...
  var salaAtual = null;
  var _listaHandler = null;
  var _jogoHandler = null;
  var _enviando = false;
  ...
  // ============================================================
  function ehMinhaVez(){
    if(!isOnlineMode || !G || G.over) return false;
    var dono = G.p1Name || '';
    var oponente = G.p2Name || '';
    if(currentUser === dono && G.turn === 0) return true;
    if(currentUser === oponente && G.turn === 1) return true;
  ...
  function aplicarEstado(data){
    ...
    var estadoStr = JSON.stringify(data);
    if(_ultimoEstado === estadoStr) return;
    _ultimoEstado = estadoStr;
    ...
    G.walls = data.paredesRestantes || G.walls;
    G.turn = (typeof data.turno === "number") ? data.turno : G.turn;
    G.over = data.status === "finalizada" || false;
  ...
  // ============================================================
  function enviarEstado(){
    if(!salaAtual || !isOnlineMode || _enviando) return;
    if(!ehMinhaVez()) return;
    _enviando = true;
    var rtdb = getRtdb();
    if(!rtdb){ _enviando = false; return; }
    var paredesH=[], paredesV=[];
    for(var i=0;i<G.pH.length;i++) paredesH.push({r:G.pH[i][0], c:G.pH[i][1], dono:G.wallOwnerH[i]||0});
    for(var j=0;j<G.pV.length;j++) paredesV.push({r:G.pV[j][0], c:G.pV[j][1], dono:G.wallOwnerV[j]||0});
    var upd = {
    ...
    rtdb.ref("salas/"+salaAtual).update(upd, function(err){
      _enviando = false;
      if(err) console.error("Erro ao enviar estado:", err);
    });
  }
  ...
  function escutarSalas(){
    ...
    rtdb.ref("salas").orderByChild("createdAt").on("value", _listaHandler);
  }
  ...
  function iniciarJogoOnline(salaId, data){
    ...
    _jogoHandler = function(snap){
      var novo=snap.val();
      if(!novo) return;
      // Não aplica se o estado veio do próprio jogador
      if(_enviando) return;
      aplicarEstado(novo);
      autoPeao();
    };
  ...
  if(typeof doMove === "function"){
    ...
    doMove = function(r,c){
      ...
      var o = _dm.apply(this, arguments);
      if(isOnlineMode) setTimeout(enviarEstado, 50);
      setTimeout(autoPeao, 100);
      return o;
    };
  }
  ...
  console.log("✅ ONLINE SINCRONIZADO ativo!");
})();
</script>
<script>
(function(){
  ...
  var _enviando = false;
  var _ultimoEstado = null;
  var souJogador1 = true;
  var _onlineProcessando = false;
  ...
  function ehMinhaVez(){
    if(!isOnlineMode || !G || G.over) return false;
    // Player 1 (criador) controla o turno 0, Player 2 controla o turno 1
    if(currentUser === G.p1Name && G.turn === 0) return true;
    if(currentUser === G.p2Name && G.turn === 1) return true;
    return false;
  }

  // ============================================================
  // AUTO SELEÇÃO DO PEÃO (IGUAL AO MODO 2P)
  // ============================================================
  function autoPeao(){
    if(!isOnlineMode || !G || G.over || !gameActive) return;
    if(!ehMinhaVez()){
    ...
    // Exatamente igual ao modo 2P: seleciona o peão do jogador atual
    var p = G.turn;
    G.sel = [G.pos[p][0], G.pos[p][1]];
    if(typeof legalMoves === "function") G.moves = legalMoves(p);
    if(typeof st === "function") st("Sua vez");
  ...
  // ============================================================
  // APLICAR ESTADO DO FIREBASE
  // ============================================================
  function aplicarEstado(data){
    if(!data || _onlineProcessando) return;
    var estadoStr = JSON.stringify(data);
    if(_ultimoEstado === estadoStr) return;
    _ultimoEstado = estadoStr;
    ...
    // Atualiza posições (sem inversão - cada um vê seu lado)
    G.pos = data.posicoes || G.pos;
  ...
  // ============================================================
  // ENVIAR ESTADO (IGUAL AO MODO 2P)
  // ============================================================
  function enviarEstado(){
    if(!salaAtual || !isOnlineMode || _enviando) return;
    if(!ehMinhaVez()) return;
  ...
  // ============================================================
  // SOBRESCREVER FUNÇÕES DO JOGO (MESMA LÓGICA DO MODO 2P)
  // ============================================================
  if(typeof doMove === "function"){
    var _dm = doMove;
    doMove = function(r,c){
  ...
  // ============================================================
  // INICIAR JOGO ONLINE (MESMA LÓGICA DO MODO 2P)
  // ============================================================
  function iniciarJogoOnline(salaId, data){
    isOnlineMode = true;
    salaAtual = salaId;
    ...
    matchFinished = false;
    _ultimoEstado = null;
    _onlineProcessando = false;

    // Define nomes (cada jogador vê seu próprio nome como Player 1)
    if(!souJogador1){
      G.p1Name = data.jogador2;
      G.p2Name = data.jogador1;
      // Inverte as posições na visão do Player 2 (ele vê seu peão embaixo)
      var pos1 = data.posicoes ? data.posicoes[0] : [8,4];
      var pos2 = data.posicoes ? data.posicoes[1] : [0,4];
      G.pos = [[pos2[0], pos2[1]], [pos1[0], pos1[1]]];
    } else {
      G.p1Name = data.jogador1;
      G.p2Name = data.jogador2;
      G.pos = data.posicoes || G.pos;
    }

    // Aplica paredes e turno
    G.walls = data.paredesRestantes || G.walls;
    ...
    _jogoHandler = function(snap){
      ...
      // Aplica o estado (com inversão para Player 2)
      if(!souJogador1){
        var pos1 = novo.posicoes ? novo.posicoes[0] : [8,4];
        var pos2 = novo.posicoes ? novo.posicoes[1] : [0,4];
        novo.posicoes = [[pos2[0], pos2[1]], [pos1[0], pos1[1]]];
      }
      aplicarEstado(novo);
      autoPeao();
    };
  ...
  console.log("✅ ONLINE (baseado no modo 2P) ativo!");
})();
</script>
<script>
(function(){
  ...
  var isOnlineMode=false, salaAtual=null, _listaH=null, _jogoH=null, _push=false;

  function db(){ try{ return firebase.database(); }catch(e){ alert(e.message); return null; } }
  function status(m){ var el=document.getElementById("sala-status"); if(el) el.innerHTML=m; }

  // MESMO tabuleiro para os dois. Vez = nome no Firebase + turno
  function minhaVez(){
    if(!isOnlineMode||!G||G.over) return false;
  ...
  function autoPeao(){
    if(!isOnlineMode||!G||G.over||!gameActive) return;
    if(!minhaVez()){
      G.sel=null; G.moves=[]; G.mode="move";
      if(typeof st==="function") st("Aguardando "+(G.turn===0?G.p1Name:G.p2Name)+"...");
      if(typeof draw==="function") draw();
      return;
    }
    G.mode="move";
    G.sel=[G.pos[G.turn][0], G.pos[G.turn][1]];
    if(typeof legalMoves==="function") G.moves=legalMoves(G.turn);
    if(typeof st==="function") st("Sua vez — toque no destino ou parede");
  ...
  function aplicar(data){
    if(!data) return;
    // nomes e estado SEMPRE absolutos (iguais nos dois celulares)
    G.p1Name=data.jogador1||G.p1Name;
    G.p2Name=data.jogador2||G.p2Name;
    if(data.posicoes) G.pos=[[data.posicoes[0][0],data.posicoes[0][1]],[data.posicoes[1][0],data.posicoes[1][1]]];
    if(data.paredesRestantes) G.walls=[data.paredesRestantes[0],data.paredesRestantes[1]];
    if(typeof data.turno==="number") G.turn=data.turno;
    ...
    var a=document.getElementById("p1-nome"); if(a) a.textContent=G.p1Name;
    var b=document.getElementById("nomeJ2"); if(b) b.textContent=G.p2Name;
    var c=document.getElementById("placar-p1"); if(c) c.textContent=G.p1Name;
    var d=document.getElementById("placar-p2"); if(d) d.textContent=G.p2Name;
    if(typeof updateWallIndicators==="function") updateWallIndicators();
    if(typeof syncBtn==="function") syncBtn();
    if(typeof draw==="function") draw();
    autoPeao();
  ...
  function enviar(){
    if(!salaAtual||!isOnlineMode||_push) return;
    var rtdb=db(); if(!rtdb) return;
    _push=true;
    var paredesH=[], paredesV=[];
    for(var i=0;i<G.pH.length;i++) paredesH.push({r:G.pH[i][0],c:G.pH[i][1],dono:G.wallOwnerH[i]||0});
    for(var j=0;j<G.pV.length;j++) paredesV.push({r:G.pV[j][0],c:G.pV[j][1],dono:G.wallOwnerV[j]||0});
    var upd={
      posicoes:[[G.pos[0][0],G.pos[0][1]],[G.pos[1][0],G.pos[1][1]]],
      paredesH:paredesH, paredesV:paredesV,
      paredesRestantes:[G.walls[0],G.walls[1]],
      turno:G.turn
    };
    if(G.over){
      upd.status="finalizada";
      upd.vencedor=(G.pos[0][0]===0)?G.p1Name:((G.pos[1][0]===8)?G.p2Name:(G.turn===0?G.p2Name:G.p1Name));
    }
    rtdb.ref("salas/"+salaAtual).update(upd, function(){ _push=false; });
  }

  if(typeof doMove==="function"){
    var _dm=doMove;
    doMove=function(r,c){
      if(isOnlineMode&&!minhaVez()){ if(typeof st==="function") st("Nao e sua vez"); return; }
      var o=_dm.apply(this,arguments);
      if(isOnlineMode) setTimeout(enviar,20);
      return o;
    };
  }
  if(typeof placeWall==="function"){
  ...
  if(canvas){
    function block(e){
      if(!isOnlineMode||!gameActive||G.over) return;
      if(!minhaVez()){ e.stopImmediatePropagation(); e.preventDefault(); }
    }
    canvas.addEventListener("click",block,true);
    canvas.addEventListener("touchstart",block,true);
  }

  function listar(){
    var rtdb=db(); if(!rtdb) return;
    if(_listaH){ try{ rtdb.ref("salas").off("value",_listaH); }catch(e){} }
    _listaH=function(snap){
      var lista=document.getElementById("sala-lista"); if(!lista) return;
      lista.innerHTML="";
      var val=snap.val()||{}, items=[];
      Object.keys(val).forEach(function(id){
      ...
      items.sort(function(a,b){ return (b.d.createdAt||0)-(a.d.createdAt||0); });
      if(!items.length){ lista.innerHTML='<div style="text-align:center;color:#666;padding:20px">Nenhuma sala. Crie uma!</div>'; return; }
      items.forEach(function(it){
        var d=it.d, nome=d.nomePartida||d.jogador1||"Sala", lock=d.senha?"🔒 ":"";
        var sou=d.jogador1===currentUser;
        var row=document.createElement("div");
        row.style.cssText="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid rgba(255,215,140,0.1)";
        row.innerHTML="<div><b style='color:#f0e6d3'>"+lock+nome+"</b><br><small style='color:#b8a99a'>"+d.jogador1+(sou?" (sua)":"")+"</small></div>";
        if(!sou){
          var btn=document.createElement("button");
          btn.textContent="Entrar";
          btn.style.cssText="padding:8px 14px;border:none;border-radius:10px;background:#4dabf7;color:#111;font-weight:800";
          btn.onclick=function(){ entrar(it.id,d); };
          row.appendChild(btn);
        }
        lista.appendChild(row);
      });
    };
    rtdb.ref("salas").on("value",_listaH);
  }
  ...
  function criar(){
    ...
    status("Criando...");
    ref.set({
      nomePartida:nome, jogador1:currentUser, jogador2:"", status:"esperando",
      turno:0, posicoes:[[8,4],[0,4]], paredesH:[], paredesV:[], paredesRestantes:[10,10],
      vencedor:"", createdAt:Date.now(), senha:senha
    }).then(function(){
      status("Sala criada. Esperando oponente...");
      alert("Sala criada!");
      isOnlineMode=true; salaAtual=id;
      ...
      listar();
    }).catch(function(err){ status(err.message); alert(err.message); });
  }

  function entrar(salaId, data){
    if(!currentUser){ alert("Faca login"); return; }
    if(data.senha){
      var s=prompt("Senha:"); if(s===null) return;
      if(s!==data.senha){ alert("Senha errada"); return; }
    }
    var rtdb=db(), ref=rtdb.ref("salas/"+salaId);
    ref.transaction(function(cur){
      if(!cur) return;
      if(cur.status!=="esperando"||(cur.jogador2&&cur.jogador2!=="")) return;
      cur.jogador2=currentUser; cur.status="em_andamento"; return cur;
    }).then(function(r){
      if(!r.committed){ alert("Sala ocupada"); listar(); return; }
      document.getElementById("sala-overlay").classList.remove("show");
      iniciar(salaId, r.snapshot.val());
    }).catch(function(err){ alert(err.message); });
  }

  function iniciar(salaId, data){
    isOnlineMode=true; salaAtual=salaId;
    G.vsIA=false; G.over=false; G.mode="move"; G.sel=null; G.moves=[];
    gameActive=true; matchFinished=false;
    aplicar(data);
    var p2c=document.getElementById("p2-controls"); if(p2c) p2c.style.display="none";
    document.getElementById("game-screen").classList.remove("p2-active","modo-2p");
    showScreen("game-screen");
    setTimeout(function(){
      if(typeof resize==="function") resize();
    ...
    var rtdb=db(), ref=rtdb.ref("salas/"+salaId);
    if(_jogoH){ try{ ref.off("value",_jogoH); }catch(e){} }
    _jogoH=function(snap){
      var novo=snap.val(); if(!novo) return;
      if(novo.status==="finalizada"){
        if(!G.over){
          G.over=true;
          if(typeof showWinOverlay==="function")
            showWinOverlay("◈ "+(novo.vencedor||"")+" venceu!", novo.vencedor===novo.jogador1?0:1);
        }
        return;
      }
      if(_push) return;
  ...
  function abrir(){
    if(!currentUser){ alert("Faca login"); return; }
    document.getElementById("sala-overlay").classList.add("show");
    status("Crie ou entre numa sala");
    listar();
  }
  function bind(id,fn){
    var el=document.getElementById(id); if(!el) return;
    var n=el.cloneNode(true); el.parentNode.replaceChild(n,el); n.addEventListener("click",fn);
  }
  bind("btn-online", abrir);
  bind("btn-criar-sala", criar);
  bind("btn-close-sala", function(){ document.getElementById("sala-overlay").classList.remove("show"); });
  bind("sala-close", function(){ document.getElementById("sala-overlay").classList.remove("show"); });
  console.log("ONLINE SYNC OK");
})();
